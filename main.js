...
const UI = {
    ...
    toggleSettings: () => { 
        ...
        if (dot && dot.classList.contains('dot-yellow')) dot.classList.add('hidden');
    },
...
// ESCUCHA DEL RADAR v1.0.3 (Motor -> Interfaz)
if (typeof require !== 'undefined') {
    ...
    ipcRenderer.on('update-status', (event, status) => {
        ...
        if (status === 'downloading') {
            ...
            btn.innerText = "⏳ DESCARGANDO PARCHE...";
...
const App = {
    ...
    // Función de actualización y botón de reinicio manual
    handleUpdateClick: () => {
        const btn = document.getElementById('btn-update-status');
