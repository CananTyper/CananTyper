...
window.CT = {
    ...
    init: function() {
        ...
        if(window.UI) window.UI.listLayout = localStorage.getItem('ct_layout') || 'layout-list';
        ...
        window.db.collection('config').doc('stats_layout').onSnapshot(snap => {
            const defLayout = {
                ...
                hc: [{id:'w-h-rec', v:true, s:3}, {id:'w-h-surv', v:true, s:3}, {id:'w-h-death', v:true, s:3}, {id:'w-h-rate', v:true, s:3}, {id:'w-h-top', v:true, s:6}, {id:'w-h-worst', v:true, s:6}, {id:'w-h-legends', v:true, s:4}, {id:'w-h-victims', v:true, s:4}, {id:'w-h-safe', v:true, s:4}]
            };
        ...
        if(this.ses()) { 
            if(window.UI && window.UI.initLobby) window.UI.initLobby(); 
        } else { 
        ...
        window.db.collection('announcements').orderBy('id', 'desc').onSnapshot(snap => { 
            ...
            if(window.UI && window.UI.checkAnnouncements) window.UI.checkAnnouncements(); 
            if(window.UI && window.UI.refreshActiveViews) window.UI.refreshActiveViews(); 
        });
