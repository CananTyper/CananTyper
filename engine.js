...
App.startGhostRace = (trackTitle, cpm) => {
...
App.startRaceWithTrack = (id) => { 
    const track = CT.dbLocal('p').find(t => t.id.toString() === id.toString()); 
    if(track) { App.currentTrack = track; if(App.activeEngine) App.activeEngine.stop(); App.activeEngine = new Engine(track, 'normal'); } 
};

App.retryRace = () => { 
    if(App.activeEngine) { 
        const m = App.activeEngine.mode; const g = App.activeEngine.ghostCPM; 
        App.activeEngine.stop(); 
        if(App.currentTrack) App.activeEngine = new Engine(App.currentTrack, m, g); 
    } 
};
...
App.quitRace = () => { 
    if(App.activeEngine) { App.activeEngine.stop(); App.activeEngine = null; } 
    UI.showLobby(); 
};
